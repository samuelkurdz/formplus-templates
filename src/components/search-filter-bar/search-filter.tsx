...
const SearchFilter = () => {
	...
	const onSelectChange = (event: ChangeEvent<HTMLSelectElement>) => {
		...
		let updatedQuery: QueryObject = {...query, [event.target.name]: event.target.value};
	...
	const getSearchNameQuery = (event: ChangeEvent<HTMLInputElement>) => {
		event.preventDefault();
		let updatedQuery: QueryObject = {...query, searchText: event.target.value};
		dispatch(queryResolver(updatedQuery));
	}
		...
		<>
			<form className="flex flex-col sm:flex-row justify-between content-center mb-10">
				<div className="form-group mb-5 sm:mb-0 relative">
					<input 
						type="text" name="name" id="name" placeholder="Search Templates"
						value={query.searchText} onChange={getSearchNameQuery}
					/>
